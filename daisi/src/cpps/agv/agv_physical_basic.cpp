...
AGVPhysicalBasic::AGVPhysicalBasic(const AgvDataModel &data_model, const TopologyNs3 &topology,
                                   int id_for_friendly, ns3::Ptr<ns3::Socket> socket)
    : agv_data_model_(data_model), socket_(socket) {
  // Topology
  ...
  // AmrDescription
  agv_data_model_.agv_properties.friendly_name.append(std::to_string(id_for_friendly));
  const uint32_t serial = agv_data_model_.agv_device_descr.serial_number;
  const AgvDeviceProperties p = agv_data_model_.agv_properties;
  const Kinematics k = data_model.agv_properties.kinematic;
  const AmrKinematics kin(k.getMaxVelocity(), k.getMinVelocity(), k.getMaxAcceleration(),
                          k.getMinAcceleration());
  const AmrLoadHandlingUnit load(k.getLoadTime(), k.getUnloadTime(), p.ability);
  const AmrPhysicalProperties phys;

  // Use default properties and only set friendly name
  const AmrProperties props(agv_data_model_.agv_properties.friendly_name);

  amr_description_ = AmrDescription(serial, kin, props, phys, load);
