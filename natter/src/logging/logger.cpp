...
void Logger::logCritical(const std::string &msg) const {
  if (log_level_ <= LogLevel::kCritical)
    log([&msg](const LoggerPtr &logger) { logger->logCritical(msg); });
}

void Logger::logWarning(const std::string &msg) const {
  if (log_level_ <= LogLevel::kWarning)
    log([&msg](const LoggerPtr &logger) { logger->logWarning(msg); });
}

void Logger::logInfo(const std::string &msg) const {
  if (log_level_ <= LogLevel::kInfo) log([&msg](const LoggerPtr &logger) { logger->logInfo(msg); });
}

void Logger::logDebug(const std::string &msg) const {
  if (log_level_ <= LogLevel::kDebug)
    log([&msg](const LoggerPtr &logger) { logger->logDebug(msg); });
}

void Logger::logNewPeer(const std::string &ip, uint16_t port, solanet::UUID uuid,
                        const std::string &topic) const {
  if (log_strings_)
    log([&ip, &port, &uuid, &topic](const LoggerPtr &logger) {
      logger->logNewPeer(ip, port, uuid, topic);
    });
}
...
void Logger::logRemovePeer(const std::string &ip, uint16_t port, solanet::UUID uuid,
  ...
  if (log_strings_)
    log([&ip, &port, &uuid, &topic](const LoggerPtr &logger) {
      logger->logRemovePeer(ip, port, uuid, topic);
    });
}
...
void Logger::logNewMessage(const std::string &topic, const std::string &msg,
  ...
  if (log_strings_)
    log([&topic, &msg, &msg_uuid](const LoggerPtr &logger) {
      logger->logNewMessage(topic, msg, msg_uuid);
    });
}

void Logger::logSendFullMsg(solanet::UUID msg_uuid, solanet::UUID uuid, solanet::UUID own_uuid) {
  if (log_strings_)
    log([&msg_uuid, &uuid, &own_uuid](const LoggerPtr &logger) {
      logger->logSendFullMsg(msg_uuid, uuid, own_uuid);
    });
}
...
void Logger::logReceiveFullMsg(solanet::UUID msg_uuid, solanet::UUID sender,
  ...
  if (log_strings_)
    log([&msg_uuid, &sender, &own_uuid](const LoggerPtr &logger) {
      logger->logReceiveFullMsg(msg_uuid, sender, own_uuid);
    });
}
...
void Logger::logMinhcastBroadcast(solanet::UUID msg_id, uint32_t level, uint32_t number,
  ...
  if (log_strings_) {
    log([&msg_id, &level, &number, &forward_up_limit,
         &forward_down_limit](const LoggerPtr &logger) {
...
void Logger::logNewNetworkPeer(solanet::UUID uuid, const std::string &ip, uint16_t port, int level,
                               int number) {
  if (log_strings_)
    log([&uuid, &ip, &port, &level, &number](const LoggerPtr &logger) {
      logger->logNewNetworkPeer(uuid, ip, port, level, number);
    });
}
...
void Logger::logReceivedMessages(solanet::UUID node_uuid, solanet::UUID initial_sender,
  ...
  if (log_strings_)
    log([&node_uuid, &initial_sender, &message, &round](const LoggerPtr &logger) {
      logger->logReceivedMessages(node_uuid, initial_sender, message, round);
    });
}

template <typename LogFunction> void Logger::log(LogFunction function) const {
