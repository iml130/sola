...
void Logger::logCritical(const std::string &msg) const {
  if (log_level_ >= LogLevel::kCritical) {
    logCommand([&msg](const LoggerPtr &logger) { logger->logCritical(msg); });
  }
}

void Logger::logWarning(const std::string &msg) const {
  if (log_level_ >= LogLevel::kWarning) {
    logCommand([&msg](const LoggerPtr &logger) { logger->logWarning(msg); });
  }
}

void Logger::logInfo(const std::string &msg) const {
  if (log_level_ >= LogLevel::kInfo) {
    logCommand([&msg](const LoggerPtr &logger) { logger->logInfo(msg); });
  }
}

void Logger::logDebug(const std::string &msg) const {
  if (log_level_ >= LogLevel::kDebug) {
    logCommand([&msg](const LoggerPtr &logger) { logger->logDebug(msg); });
  }
}

void Logger::logPhysicalNodeInfo(const LoggerPhysicalNodeInfo &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logPhysicalNodeInfo(info); });
}

void Logger::logNode(const LoggerInfoAddNode &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logNode(info); });
}

void Logger::logNodeUninit(const LoggerInfoNodeState &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logNodeUninit(info); });
}

void Logger::logNodeRunning(const LoggerInfoNodeState &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logNodeRunning(info); });
}

void Logger::logNodeLeft(const LoggerInfoNodeState &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logNodeLeft(info); });
}

void Logger::logNeighbor(const LoggerInfoAddNeighbor &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logNeighbor(info); });
}

void Logger::logEvent(const LoggerInfoAddEvent &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logEvent(info); });
}

void Logger::logSearchExactTest(const LoggerInfoSearchExact &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logSearchExactTest(info); });
}

void Logger::logTraffic(const MessageLoggingInfo &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logTraffic(info); });
}

void Logger::logContent(const LoggerInfoAddContent &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logContent(info); });
}

void Logger::logFindQuery(const LoggerInfoAddFindQuery &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logFindQuery(info); });
}

void Logger::logFindQueryResult(const LoggerInfoAddFindQueryResult &info) {
  logCommand([&info](const LoggerPtr &logger) { logger->logFindQueryResult(info); });
}

template <typename LogFunction> void Logger::logCommand(LogFunction function) const {
