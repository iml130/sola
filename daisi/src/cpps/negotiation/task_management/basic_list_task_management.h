...
#ifndef DAISI_NEGOTIATION_BASIC_LIST_TASK_MANAGEMENT_NS3_H_
#define DAISI_NEGOTIATION_BASIC_LIST_TASK_MANAGEMENT_NS3_H_

#include "cpps/negotiation/task_management/task_management.h"

namespace daisi::cpps {

class BasicListTaskManagement : public TaskManagement {
public:
  BasicListTaskManagement(const std::shared_ptr<Kinematics> &kinematics,
  ...
  virtual void updateQueue(std::list<std::pair<Task, UtilityDimensions>>::iterator &it);
...
protected:
  std::list<std::pair<Task, UtilityDimensions>> queue_;
};
